...
The public app contains no league history. Users may import local draft seasons, manager IDs and aliases, final rosters, projected keepers, and optional player metadata through a JSON league profile. Imported picks build manager profiles from position rates in early, middle, and late round bands; typical quarterback and tight-end timing; repeated-player selections; NFL-team frequency; player archetypes; and roster-construction patterns. With no imported history, every manager regresses to the league and market baseline.

Archetype signals are manager-vs-league comparisons, not raw labels. The engine currently measures rookie, second-year, position-adjusted young/veteran, late youth, physical frame, NFL draft capital, dated durability, and dated role clarity when the necessary metadata exists. Each rate is shrunk toward the league baseline using eligible sample size, manager-identity confidence, and metadata coverage. The local reviewed player catalog can reconstruct age/experience for matched active players; unmatched historical players remain outside the denominator and coverage is displayed. This avoids silently treating missing records as “not a rookie” or “not an injury risk.”

Roster-construction signals use the complete draft rows and therefore have broader coverage: same-position picks in consecutive rounds, multiple players from one NFL team, QB plus WR/TE stacks, same-team RB pairs, favorite-team concentration, and repeated-player loyalty. Supported player-type and stacking preferences can make small, capped adjustments between candidates already in the same plausible market range. They cannot override position need, roster legality, or a large rank gap.

Injury appetite and ambiguous-role appetite require evidence dated to the historical draft. Current durability, later injuries, later depth-chart outcomes, and end-of-season role are never backfilled into an old decision. Optional `playerMetadata` may provide `durabilityByYear` and `roleClarityByYear` on a 0–100 scale, plus stable age, size, and NFL draft-capital fields. Unsupported categories are shown as unscored rather than inferred.

The “Returns” estimate begins with the player's current baseline rank and uncertainty around that expected selection point. It then conditions on every open opponent pick before the user's next snake selection. Each intervening pick is adjusted for that manager's historical position rate, supported archetype fit, current roster construction, prior selection of the same player, and whether a projected keeper occupies the pick. These percentages are directional decision support, not statistically calibrated guarantees.
